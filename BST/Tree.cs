...
namespace BST {
  public class Tree {
    ...
    public static int getDistance (Node root, int node, int distance) {
      if (root == null) {
        return -1;
      }
    ...
    public static int distanceToRoot (Tree tree, int value) {
      ...
      return getDistance (root, value, 0);
    }
    ...
    public static int distanceBetweenNodes (Tree tree, int a, int b) {
      ...
      if (root.getValue () == a) {
        return getDistance (root, b, 0);
      }
      if (root.getValue () == b) {
        return getDistance (root, a, 0);
      }
      ...
      if (areInDifferentBranches) {
        return getDistance (root, a, 0) + getDistance (root, b, 0);
      }
